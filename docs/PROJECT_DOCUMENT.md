...
| 2026-02-17 | Phase 2 Type & API Reliability | Added shared API DTOs (`Project`, `Event`, `Task`, `TeamMember`), replaced `any` in shared API/auth boundaries with typed generics, and introduced normalized API error handling used by core pages (`Auth`, `Projects`, `Team`, `Tasks`). | `npx eslint ...`, `npm run build` | AI agent |
